...
export async function scrapeElPeruanoNoticiasDeSeccion(page, url, limit = 20) {
  

  

  // Scroll a fixed number of times with a fixed wait to load more news
  const fixedScrolls = 5; // Scroll 5 times
  const fixedWaitTime = 6000; // Wait 6 seconds

  for (let i = 0; i < fixedScrolls; i++) {
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    await new Promise(res => setTimeout(res, fixedWaitTime));
  }

  // After scrolling, wait a bit more for the last batch to fully render
  await new Promise(res => setTimeout(res, 2000)); // Additional wait after all scrolls
  ...
  const news = (await page.evaluate((url, limit) => {
    ...
    const articles = Array.from(section.querySelectorAll('article')).filter(article => !article.querySelector('.sknota'));
    ...
    try {
      ...
      for (const item of news) {
        ...
        try {
          ...
          // Increased timeout to wait longer for content after skeleton disappears
          await detailPage.waitForFunction(() => !document.querySelector('.skeleton-nota'), { timeout: 15000 }).catch(() => { console.log(`Timeout waiting for content on ${item.url}`) });
          detalle = await detailPage.evaluate(() => {
            const titulo = document.querySelector('main#portada h1')?.textContent?.trim() || '';
            let contenidoHTML = document.querySelector('#contenido')?.innerHTML || '';
  
            // Optionally, extract a plain text subtitle from the beginning of the content for summary purposes
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = contenidoHTML;
            const firstP = tempDiv.querySelector('p, div > span > div > strong'); // Try finding first paragraph or relevant strong tag
            const subtituloExtraido = firstP?.textContent?.trim().split(/[.,]/)[0] || ''; // Extract text up to first period/comma
  
            // Try to pretty print the HTML using js_beautify if available
            if (window.js_beautify) {
              contenidoHTML = window.js_beautify(contenidoHTML, {
                indent_size: 2,
          ...
          console.error(`Error scraping detail page ${item.url}: ${error.message}`);
      ...
      if (detailPage) {
        try { await detailPage.close(); } catch (e) { console.error("Error closing detail page:", e); }
      }
      // if (page) {
      //   try { await page.close(); } catch (e) { console.error("Error closing main page:", e); }
      // }
      // if (browser) {
...
export async function scrapeDeporCategoriaCompleto(url, limit = 20) {
  ...
  let detailPage = null; // Initialize detailPage to null
  const results = []; // Initialize results array

  try { // Wrap main logic in try block
    page = await browser.newPage(); // Assign page inside try
    ...
    // Procesar detalles usando una única página reutilizada
    detailPage = await browser.newPage(); // Create detailPage once before the loop

    for (const item of items) {
      if (!item.url) continue;
      let detail = { section: '', title: '', subtitle: '', date: '', img: '', content: '' };
      try {
        // Navigate the *same* detailPage
        await detailPage.goto(item.url, { waitUntil: 'domcontentloaded', timeout: 15000 }); // Increased timeout
        await detailPage.waitForSelector('.sht__title', { timeout: 7000 }); // Increased timeout
      ...
      // }); <-- REMOVED EXTRA CLOSING CHARACTERS

      // DO NOT close detailPage here
      // Lógica para subtitulo: si no hay, usar summary o el resumen hasta la primera coma
      let subtituloFinal = detail.subtitle || item.subtitle || ''; // Use let
      if (!subtituloFinal) {
      ...
      // Moved catch block to be correctly associated with the inner try
     } catch (err) { // Catch specific error for detail page scraping
        console.error(`Error scraping detail page ${item.url}: ${err.message}`);
        // DO NOT close detailPage here
        results.push({
          ...item,
          content: '', // Add empty content on error
          error: err.message // Optionally add error message
        });
      }
    } // End of for loop
  } // End of outer try block
  finally { // Add finally block for browser/page cleanup
    if (detailPage) {
      try { await detailPage.close(); } catch (e) { console.error("Error closing detail page:", e); }
    }
    if (page) {
