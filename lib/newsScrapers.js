...
export async function scrapeJornadaCategoria(page, url, limit = 20) {
  await page.goto(url, { waitUntil: 'domcontentloaded' });
...
export async function scrapeJornadaCategoriaSimple(url, limit = 20) {
  const browser = await launchBrowser();
  const page = await browser.newPage();
  try {
    const result = await scrapeJornadaCategoria(page, url, limit);
    await browser.close();
    return result;
  } catch {
    await browser.close();
...
export { readCache, writeCache, formatDateToIsoPeru, scrapeJornadaCategorias };
