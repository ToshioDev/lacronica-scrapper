...
export async function scrapeElPeruanoNoticiasDeSeccion(url, limit = 20) {
  ...
  // Esperar a que carguen al menos 'limit' artículos reales (no skeletons)
  ...
  // Limita el número de noticias a 'limit'
  ...
  const concurrency = Math.min(10, Math.max(1, Math.floor(limit / 4) || 1));
  const results = [];
  let idx = 0;
    ...
    const batchResults = await Promise.allSettled(batch.map(async (item) => {
      ...
      try {
        ...
        detalle = await pageDetalle.evaluate(() => {
          ...
          return { titulo, subtitulo, contenido: contenidoHTML };
        });
...
export async function scrapeDeporCategoriaCompleto(url, limit = 20) {
  ...
  const concurrency = 8; // Número de tabs en paralelo para categorías
  const results = [];
  let idx = 0;
  async function processBatch(batch) {
    const batchResults = await Promise.allSettled(batch.map(async (item) => {
  ...
  try {
    ...
    async function processBatch(batch) {
      const batchResults = await Promise.all(batch.map(async art => {
        ...
        let detail = { titulo: '', subtitulo: '', contenido: '' };
        const detailPage = await browser.newPage();
        try {
          ...
          detail = await detailPage.evaluate(() => {
            ...
            const subtitulo = document.querySelector('.tdb_single_subtitle .tdb-block-inner')?.textContent?.trim() || '';
            let contenido = '';
...
async function scrapeJornadaDetalle(page, url) {
  ...
  const subtitulo = await page.evaluate(() => {
    const sub = document.querySelector('.tdb_single_subtitle .tdb-block-inner')?.textContent?.trim() || '';
    return sub;
  });
  ...
  // Format as left-justified HTML (convert newlines to <br> and wrap in a <div style="text-align:left">)
  const contenidoHTML = `<div style=\"text-align:left\">${contenidoSinFechas.replace(/\n/g, '<br>')}</div>`;
  return { content: contenidoHTML, date, titulo, subtitulo };
}
