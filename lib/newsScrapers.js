...
export async function scrapeJornadaDetalle(page, url) {
  ...
  const date = await page.evaluate(() => {
    const dateEl = document.querySelector('.td-single-date, .tdb_single_date time, time.td-module-date')?.getAttribute('datetime') ||
      document.querySelector('.td-single-date, .tdb_single_date time, time.td-module-date')?.textContent?.trim() || '';
    return dateEl;
  });
